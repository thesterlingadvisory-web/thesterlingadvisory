    ...
    <div className="w-full bg-primary min-h-screen">
      ...
      <section className="pt-24 pb-16 px-6 text-center">
        <div className="max-w-3xl mx-auto">
          <h1 className="text-5xl md:text-6xl font-heading mb-6 tracking-tight">Our Capabilities</h1>
      ...
      <section className="pb-32 px-6 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {serviceCategories.map((category) => (
            <div key={category.id} className="bg-white p-8 md:p-10 border border-border-main rounded-sm hover:shadow-xl hover:shadow-black/5 transition-all duration-500">
              <div className="flex items-start md:items-center gap-4 mb-8 flex-col md:flex-row">
                <div className="w-14 h-14 bg-primary flex items-center justify-center text-accent shrink-0 rounded-sm">
              ...
              <div className="flex flex-wrap gap-2.5">
                {category.services.map((service) => (
                  <Link 
                    key={service.id} 
                    to={`/capabilities/${service.slug}`}
                    className="inline-flex items-center px-4 py-2 text-sm font-medium border border-border-main text-text-main bg-primary/30 hover:bg-text-main hover:text-white hover:border-text-main transition-all duration-300 rounded-sm"
                  >
                    {service.title}
                  </Link>
                ))}
